...
//    private var currentCategory: String? = "Новая категория"
    ...
    @objc private func createButtonTapped() {
        dismiss(animated: true) {
            self.delegate?.addNewTrackerCategory(TrackerCategory(title: self.lastCategory!, trackers: [Tracker(id: UUID(), text: self.trackerText, emoji: self.emoji, color: self.color, schedule: self.schedule)]))
        }
    }
    
    private func buttonIsEnabled() {
        guard let text = textField.text, let lastCategory else { return }
        if !text.isEmpty && !lastCategory.isEmpty {
...
extension NewTrackerViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        switch indexPath.row {
        case 0:
            guard let categoriesVC = CategoriesAssembly().assemble(with: CategoryConfiguration(lastCategory: lastCategory)) as? CategoriesViewController else  {return}
